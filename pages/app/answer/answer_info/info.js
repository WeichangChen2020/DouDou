...
Page({
  data: {
    ...
    storeUrl: '1.0?method=pingshifen.question.collect&course_id=' + wx.getStorageSync('pingshifen_current_course_id'), // 收藏url
    answerUrl: '1.0?method=pingshifen.question.submit&course_id=' + wx.getStorageSync('pingshifen_current_course_id'), //提交答案URL
    answers: {
      isShowRemove: false, //是否显示移除按钮
      onLoadUrl: '1.0?method=pingshifen.question.get_id_items&course_id=' + wx.getStorageSync('pingshifen_current_course_id'), //题目号链接    
      start: 0, //初始题号
      end: 0, //结束题号
      allLists: [], //题号数据
      activeNum: 0, //当前条数
      ...
      url: '1.0?method=pingshifen.question.get_info&course_id=' + wx.getStorageSync('pingshifen_current_course_id'), //题目详情链接
      isShowTip: false //默认是否显示提示
    },
    autoJump: true, //是否自动跳转到下一题
  },
  //单选逻辑
  tapRadio: function(e) {
    //判断是否为已答题
  ...
  callBackGetSubject: function(d, start, end) {
    ...
    d.data.forEach((data, i) => {
      if (!!this.data.answers.allLists[start + i]) {
